...
namespace TaskbarTiles
{
    sealed partial class SettingsWindow
    {
        ...
        void AddStreamDockPage()
        {
            ...
            var body = new TableLayoutPanel { Dock = DockStyle.Fill, Padding = new Padding(14), ColumnCount = 1, RowCount = 6 };
            ...
            body.Controls.Add(new Label { Dock = DockStyle.Fill, ForeColor = Theme.Text, Text = "Optional Stream Dock modules\r\nTick the actions you want available. Close Stream Dock before applying. Your taskbar behaviour and other plugins stay unchanged." }, 0, 0);
            ...
            dockGrid.RowTemplate.Height = 32; dockGrid.ColumnHeadersHeight = 34;
            dockGrid.Columns.Add(new DataGridViewCheckBoxColumn { Name = "Available", HeaderText = "On", Width = 48 });
            dockGrid.Columns.Add(new DataGridViewTextBoxColumn { Name = "Module", HeaderText = "Module", ReadOnly = true, AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill, FillWeight = 55 });
            ...
            dockGrid.Columns.Add(new DataGridViewTextBoxColumn { Name = "Installed", HeaderText = "Stream Dock", ReadOnly = true, AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill, FillWeight = 45 });
            ...
            dockApply = Theme.Button("Apply Stream Dock choices", 230); dockApply.Click += delegate { ApplyDockChoices(false); };
            dockRepair = Theme.Button("Repair enabled packages", 200); dockRepair.Click += delegate { ApplyDockChoices(true); };
            var folder = Theme.Button("Plugin folder", 125); folder.Click += delegate { try { Directory.CreateDirectory(DockManager.DefaultRoot); Process.Start(new ProcessStartInfo("explorer.exe", "\"" + DockManager.DefaultRoot + "\"") { UseShellExecute = true }); } catch (Exception ex) { dockStatus.Text = ex.Message; } };
            var refresh = Theme.Button("Refresh list", 120); refresh.Click += delegate { if (!dockBusy) LoadDockChoices(); };
            ...
            body.Controls.Add(new Label { Dock = DockStyle.Fill, ForeColor = Theme.Muted, Text = "After Apply, reopen Stream Dock. Drag enabled actions from FoughtApple under Key or Info board.\r\nThese choices use their own Apply button; the general Settings Apply/Cancel does not install or uninstall plugins. Off preserves settings and existing placements may show an unavailable action until re-enabled." }, 0, 5);
