...
namespace TaskbarTiles
{
    static class SettingsHelp
    {
        static readonly Dictionary<string, string> hints = new Dictionary<string, string> {
            ...
            { "ShowNotificationArea", "Show a compact mirror of Windows notification-area/tray items between Taskbar apps and the footer. Taskbar auto-hide does not intentionally hide this row. Left-click requests the real item's default action; right-click opens Taskbar Tiles actions." },
            { "NotificationShowAllItems", "Enabled by default. Include hidden/overflow notification-area items as well as currently visible tray items. Turn this off to keep only items Windows reports as visible on the taskbar." },
