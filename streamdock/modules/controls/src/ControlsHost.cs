// Native Stream Dock adapter for the already-tested direct controls. Idle unless pressed.
using System;
...
namespace FoughtAppleControls
{
    static class Host
    {
        ...
        static async Task Run(string[] args)
        {
            ...
            if (args.Contains("--validate")) { foreach (string a in allowed) if (!Files.ContainsKey(a) || !File.Exists(Path.Combine(Root, "plugin", Files[a]))) throw new IOException("Missing action executable."); return; }
            int port; string value, uuid, registration;
            if (!options.TryGetValue("port", out value) || !int.TryParse(value, out port) || port < 1 || port > 65535 || !options.TryGetValue("pluginUUID", out uuid) || !options.TryGetValue("registerEvent", out registration)) throw new IOException("Add Desktop Controls from Stream Dock's Key tab, not Toolbox Open.");
            using (Socket = new ClientWebSocket())
            {
                ...
                while (Socket.State == WebSocketState.Open)
                {
                    using (var message = new MemoryStream())
                    {
                        ...
                        do { part = await Socket.ReceiveAsync(new ArraySegment<byte>(buffer), CancellationToken.None); if (part.MessageType == WebSocketMessageType.Close) return; message.Write(buffer, 0, part.Count); if (message.Length > 262144) throw new IOException("Oversized host message."); } while (!part.EndOfMessage);
                        ...
                        if (ev == "willDisappear") { Contexts.Remove(ctx); continue; }
                        if (!allowed.Contains(action)) continue;
                        if (ev == "willAppear") { Contexts[ctx] = action; continue; }
                        try {
                            if (ev == "keyDown" && Contexts.ContainsKey(ctx) && Contexts[ctx] == action) Launch(action, Path.Combine(Root, "plugin", Files[action]), "");
                                ...
                                var payload = e["payload"] as Dictionary<string, object>; object command;
                                if (payload != null && payload.TryGetValue("command", out command)) {
                                    if ((string)command == "test" && action == "com.foughtapple.controls.screenshot") Launch(action, Path.Combine(Root, "plugin", Files[action]), "--test");
                                    if ((string)command == "setup" && (action == "com.foughtapple.controls.rocket" || action == "com.foughtapple.controls.overwatch")) {
                        ...
                        } catch (Exception ex) { Log(ex.Message); await Send(new { @event = "showAlert", context = ctx }); }
                    }
                }
            }
        }
